...
- **설명:** 브랜치의 모든 변경 내역을 하나의 커밋으로 압축하여 병합합니다.
- **장점:** 히스토리가 간결해지고, 불필요한 커밋을 제거할 수 있습니다.
- **단점:** 개별 커밋 히스토리가 사라지므로 디버깅에 어려움이 있을 수 있습니다.

# ReservationService

## Create

메소드에서 너무 많은 기능을 담고있음

함수의 역할이 뭔지 불분명함

알림을 create에서 보낼 필요가 없다 → 어차피 비동기라 관리가 안되는데 왜 포함되어 있는가?

create가 아니고 역할에 맞는 이름이 있을 것 같다

직관적인 느낌을 줘야하는데 어디서 어디까지가 뭘 하는 코드인지가 이해가 안됨

추상화를 한번 더 하는게 좋을 것 같음(관점의 차이니까 고민해볼 것)

## Static MAX_SEATS

4매까지 예매가능 이런 숫자 전부 상수처리

이런 것들이 쌓이면 다른 개발자에게 설명을 하는데에 내 시간을 써야함
...
회색 영역을 찾아가면서 일하는 사람인지

분업화된 각자 업무에 초점이 맞는 대기업형 사람인지

자기 마음속의 최저점을 인지할 것 

싫어하는 것과 하고싶은 것, 도달하고 싶은 목표, 싫어도 할 수 있는것

# TossPayments

클라이언트키가 왜 멱등해야 하는지

A대한 요청을 보내야하는데 이게 왜 유니크 해야하는지

→ 중복결제 요청이 들어갈 수 있기 때문입니다
...
Request와 DTO 네이밍의 충돌

기능이 추가되면 dto가 너무 많아지지 않는가?

# Test Code

리팩토링하면서 테스트가 깨지면 안된다!

“구현 세부사항이랑 결합하지 말라”

객체지향 SOLID원칙과 연관지어 수정하지 말고 확장하라~

내부로직에 관계없이 테스트 코드가 깨지지 않도록!

# Mail

500에러 발생시

계속 재시도 하는 로직 몇번 시도 할건지도 생각해야함

재발송 시간대에 다시 시도 하거나

하는 방법 등을 생각해봄

뭔가 실패했을 때 재시도하는 로직이 필요

이게 과도하게 실행될 수도 있으니까 백오프 전략이 필요하다
